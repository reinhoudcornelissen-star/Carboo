...
def render_landing_page():
    ...
    # Knoppen in de landing page sturen signaal via query params
    # We zetten de knoppen om naar ?actie=login of ?actie=register
    st.markdown("""
    <style>
    ...
    with col2:
        ...
        if st.button("▶  Aan de slag — Gratis registreren", key="landing_register", use_container_width=True, type="primary"):
            st.session_state["toon_landing"] = False
            st.session_state["_login_tab"]   = "register"
            st.rerun()
        if st.button("Inloggen →", key="landing_login", use_container_width=True):
            st.session_state["toon_landing"] = False
    ...
    try:
        import os
        html_path = os.path.join(os.path.dirname(__file__), "carboo_landing.html")
        with open(html_path, encoding="utf-8") as f:
            html_content = f.read()
        # Vervang knoppen zodat ze via query params werken
        html_content = html_content.replace(
            'href="https://carboo.onrender.com"',
            'href="javascript:void(0)" onclick="top.location.href=\'?actie=register\'"'
        )
        components.html(html_content, height=7500, scrolling=True)
    except Exception as e:
        st.error(f"Landing page niet gevonden: {e}")
        st.session_state["toon_landing"] = False
        st.rerun()
